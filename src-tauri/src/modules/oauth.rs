...
// Google OAuth configuration — set via build-time env vars or tauri.conf.json
const CLIENT_ID: &str = match option_env!("GOOGLE_OAUTH_CLIENT_ID") {
    Some(value) => value,
    None => "",
};
const CLIENT_SECRET: &str = match option_env!("GOOGLE_OAUTH_CLIENT_SECRET") {
    Some(value) => value,
    None => "",
};
