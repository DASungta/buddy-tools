...
pub fn create_tray(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    // 4. Build menu
    // CodeBuddy 子菜单项
    let cb_accounts = crate::modules::codebuddy_cn_account::list_accounts_checked().unwrap_or_default();
    let cb_current_id = {
        let cfg = crate::modules::config::load_app_config().unwrap_or_default();
        cfg.proxy.codebuddy_cn.current_account_id.clone()
    };
    let cb_label = if cb_accounts.is_empty() {
        "CodeBuddy: --".to_string()
    } else {
        let cur = cb_accounts.iter()
            .find(|a| Some(&a.id) == cb_current_id.as_ref())
            .or_else(|| cb_accounts.first());
        cur.map(|a| format!("CodeBuddy: {}", a.email.chars().take(20).collect::<String>()))
           .unwrap_or_else(|| "CodeBuddy: --".to_string())
    };
    let cb_info = MenuItem::with_id(app, "cb_info", &cb_label, false, None::<&str>)?;
    let cb_refresh = MenuItem::with_id(app, "cb_refresh", "刷新配额", true, None::<&str>)?;
    let sep_cb = PredefinedMenuItem::separator(app)?;
        ...
        .on_menu_event(move |app, event| {
            ...
            match event.id().as_ref() {
                ...
                "cb_refresh" => {
                    ...
                    tauri::async_runtime::spawn(async move {
                        let _ = crate::modules::codebuddy_cn_account::refresh_all_tokens().await;
                        crate::modules::tray::update_tray_menus(&app_c);
                    });
                }
                _ => {}
            }
...
pub fn update_tray_menus(app: &tauri::AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
         ...
         let cb_info_item = MenuItem::with_id(&app_clone, "cb_info", &cb_label, false, None::<&str>);
         let cb_refresh_item = MenuItem::with_id(&app_clone, "cb_refresh", "刷新配额", true, None::<&str>);
         ...
         if let (Ok(i_u), Ok(s_n), Ok(r_c), Ok(s), Ok(q), Ok(cb_i), Ok(cb_r)) = (info_user, switch_next, refresh_curr, show_i, quit_i, cb_info_item, cb_refresh_item) {
             ...
             let sep_cb = PredefinedMenuItem::separator(&app_clone).ok();
             ...
             items.push(&cb_i);
             items.push(&cb_r);
             if let Some(ref s) = sep_cb { items.push(s); }
             items.push(&s_n);
             items.push(&r_c);
