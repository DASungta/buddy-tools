...
// 导出 user_token 命令
pub mod user_token;
// 导出 codebuddy CN 多账号命令
pub mod codebuddy_cn;
// 导出 codebuddy CN 多实例命令
...
pub async fn add_account(
    ...
    let service = modules::account_service::AccountService::new(
        crate::modules::system_integration::SystemManager::Desktop(app.clone()),
    );
...
pub async fn start_oauth_login(app_handle: tauri::AppHandle, oauth_client_key: Option<String>) -> Result<Account, String> {
    ...
    let service = modules::account_service::AccountService::new(
        crate::modules::system_integration::SystemManager::Desktop(app_handle.clone()),
    );
...
pub async fn open_data_folder() -> Result<(), String> {
    let path = modules::app_paths::get_data_dir()?;
