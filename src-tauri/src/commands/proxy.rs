...
pub async fn start_proxy_service(
    ...
    internal_start_proxy_service(
        config,
        &state,
        crate::modules::system_integration::SystemManager::Desktop(app_handle),
...
pub async fn internal_start_proxy_service(
    ...
    // Ensure monitor exists
    {
        ...
        if monitor_lock.is_none() {
            let app_handle =
                if let crate::modules::system_integration::SystemManager::Desktop(ref h) = integration {
                    Some(h.clone())
                } else {
                    None
                };
...
pub async fn ensure_admin_server(
    ...
    let app_data_dir = crate::modules::app_paths::get_data_dir()?;
