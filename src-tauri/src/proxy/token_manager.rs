...
impl TokenManager {
                    ...
                    if now >= token.timestamp - 90 {
                        ...
                        if let Some(latest) = latest_token_opt {
                            if now < latest.timestamp - 90 {
                                ...
                                match crate::proxy::common::utils::refresh_proxy_oauth_access_token(&token.refresh_token, Some(&token.account_id))
                                    .await
                                {
                                    Ok(token_response) => {
            ...
            if now >= token.timestamp - 90 {
                ...
                if let Some(latest) = latest_token_opt {
                    if now < latest.timestamp - 90 {
                        ...
                        // 调用 OAuth 刷新 token
                        match crate::proxy::common::utils::refresh_proxy_oauth_access_token(&token.refresh_token, Some(&token.account_id)).await {
                            Ok(token_response) => {
    ...
    /// 保存刷新后的 token 到账号文件
    async fn save_refreshed_token(&self, account_id: &str, token_response: &crate::proxy::common::utils::ProxyOauthToken) -> Result<(), String> {
        let entry = self.tokens.get(account_id)
            .ok_or("账号不存在")?;
    ...
    pub async fn get_token_by_email(
        ...
        // 调用 OAuth 刷新 token
        match crate::proxy::common::utils::refresh_proxy_oauth_access_token(&refresh_token, Some(&account_id)).await {
            Ok(token_response) => {
    ...
    pub async fn exchange_code(&self, code: &str, redirect_uri: &str) -> Result<String, String> {
        crate::proxy::common::utils::exchange_proxy_oauth_code(code, redirect_uri)
            .await
            .and_then(|t| {
                t.refresh_token
    ...
    pub fn get_oauth_url_with_redirect(&self, redirect_uri: &str, state: &str) -> String {
        crate::proxy::common::utils::get_proxy_oauth_url(redirect_uri, state)
    }
    ...
    pub async fn get_user_info(
        &self,
        refresh_token: &str,
    ) -> Result<crate::proxy::common::utils::ProxyOauthUserInfo, String> {
        crate::proxy::common::utils::fetch_proxy_oauth_user_info_from_refresh_token(refresh_token)
            .await
    }
    ...
    pub async fn add_account(&self, email: &str, refresh_token: &str) -> Result<(), String> {
        ...
        let token_info = crate::proxy::common::utils::refresh_proxy_oauth_access_token(refresh_token, None)
            .await
        ...
        // 3. 委托给 proxy common helper 处理磁盘写入
        let email_clone = email.to_string();
        let refresh_token_clone = refresh_token.to_string();

        tokio::task::spawn_blocking(move || {
            crate::proxy::common::utils::upsert_proxy_account(
                email_clone,
                refresh_token_clone,
                token_info,
                project_id,
    ...
    pub async fn set_forbidden(&self, account_id: &str, reason: &str) -> Result<(), String> {
        ...
        crate::proxy::common::utils::mark_proxy_account_forbidden(account_id, reason)?;
