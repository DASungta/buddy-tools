...
fn default_codebuddy_base_url() -> String {
    "https://copilot.tencent.com".to_string()
}

fn default_codebuddy_model() -> String {
    "glm-5.1".to_string()
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "snake_case")]
pub enum CodeBuddyDispatchMode {
    /// Never use CodeBuddy.
    Off,
    /// Use CodeBuddy for all OpenAI protocol requests.
    Exclusive,
    /// Use CodeBuddy only when the main pool is unavailable.
    Fallback,
    /// Treat CodeBuddy as one additional slot in the shared pool.
    Pooled,
}
...
pub struct CodeBuddyConfig {
    #[serde(default)]
    pub enabled: bool,
    #[serde(default = "default_codebuddy_base_url")]
    pub base_url: String,
    /// Keycloak JWT Bearer token for CodeBuddy auth.
    #[serde(default)]
    pub token: String,
    /// X-User-Id header value (extracted from JWT sub claim).
    #[serde(default)]
    pub user_id: String,
    #[serde(default)]
    pub dispatch_mode: CodeBuddyDispatchMode,
    /// Default model to use when forwarding requests.
    #[serde(default = "default_codebuddy_model")]
    pub model: String,
    /// ID of the currently active multi-account entry.
    /// When set, proxy uses credentials from `codebuddy_accounts/{id}.json`
    /// instead of the legacy `token`/`user_id` fields.
    #[serde(default)]
...
pub struct ProxyConfig {
    ...
    /// CodeBuddy CN (www.codebuddy.cn) provider configuration.
    #[serde(default)]
    pub codebuddy_cn: CodeBuddyConfig,
...
impl Default for ProxyConfig {
    fn default() -> Self {
        Self {
            ...
            enable_logging: true, // 默认开启，支持请求日志与监控
            debug_logging: DebugLoggingConfig::default(),
            upstream_proxy: UpstreamProxyConfig::default(),
            zai: ZaiConfig::default(),
            codebuddy_cn: CodeBuddyConfig::default(),
