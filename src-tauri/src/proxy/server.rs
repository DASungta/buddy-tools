...
pub struct AppState {
    ...
    pub codebuddy_cn: Arc<RwLock<crate::proxy::CodeBuddyConfig>>,
    ...
    pub integration: crate::modules::system_integration::SystemManager, // [NEW] 系统集成层实现
...
pub struct AxumServer {
    ...
    codebuddy_cn_state: Arc<RwLock<crate::proxy::CodeBuddyConfig>>,
...
impl AxumServer {
    ...
    pub async fn update_codebuddy_cn(&self, config: &crate::proxy::config::ProxyConfig) {
        let mut cb = self.codebuddy_cn_state.write().await;
        *cb = config.codebuddy_cn.clone();
        tracing::info!("CodeBuddy CN 配置已热更新");
    }
    ...
    proxy_pool_manager.clone().start_health_check_loop();
        ...
        let codebuddy_cn_state = Arc::new(RwLock::new(codebuddy_cn_config));
        ...
        let state = AppState {
            ...
            zai: zai_state.clone(),
            codebuddy_cn: codebuddy_cn_state.clone(),
...
async fn admin_list_oauth_clients(
    ...
    let clients = crate::proxy::common::utils::list_proxy_oauth_clients_json().map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
...
async fn admin_get_active_oauth_client(
    ...
    let client_key = crate::proxy::common::utils::get_active_proxy_oauth_client_key().map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
...
async fn admin_set_active_oauth_client(
    ...
    crate::proxy::common::utils::set_active_proxy_oauth_client_key(&payload.client_key).map_err(|e| {
        (
...
async fn admin_get_data_dir_path() -> impl IntoResponse {
    match crate::modules::app_paths::get_data_dir() {
    ...
    tokio::spawn(async move {
        match code_rx.recv().await {
            Some(Ok(code)) => {
                ...
                // 为 Web 回调提供简化的后端处理流程
                match crate::proxy::common::utils::exchange_proxy_oauth_code(&code, &redirect_uri_clone).await {
                    Ok(token_resp) => {
