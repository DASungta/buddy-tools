            ...
            let mut response = if status.is_success() {
                ...
                if status_code == 403 {
                    let resolved_account_id = if !account_id.is_empty() {
                        ...
                        // 尝试通过 email 查找账号 ID
                        crate::proxy::common::utils::find_proxy_account_id_by_email(&req.email)
                            .unwrap_or_default()
                    };
                    ...
                    if !resolved_account_id.is_empty() {
                        ...
                        let _ = crate::proxy::common::utils::mark_proxy_account_forbidden(&resolved_account_id, &error_text);
                    } else {
                        warn!(
