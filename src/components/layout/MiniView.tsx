...
export default function MiniView() {
    ...
    const currentAccount: any = null;
    const refreshQuota = async (_id: string) => {};
    ...
    const geminiProModel = (currentAccount?.quota?.models as any[] | undefined)
        ?.filter((m: any) =>
    ...
    const geminiFlashModel = (currentAccount?.quota?.models as any[] | undefined)?.find((m: any) => m.name.toLowerCase() === 'gemini-3-flash');

    const claudeGroupNames = [
        'claude-opus-4-6-thinking',
        'claude'
    ];
    const claudeModel = (currentAccount?.quota?.models as any[] | undefined)
        ?.filter((m: any) => claudeGroupNames.includes(m.name.toLowerCase()))
        .sort((a: any, b: any) => (a.percentage || 0) - (b.percentage || 0))[0];
